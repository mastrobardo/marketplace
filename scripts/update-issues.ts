...
if (!repo || repo.startsWith('--')) {
  console.error('need --repo <owner>/<name>');
  process.exit(1);
}
...
if (/iagl|parserdigital/i.test(who)) {
  console.error(`✖ refusing: gh identity is ${who}`);
  process.exit(1);
}
...
const EPICS: [string, string, string][] = [
  [
    ...
    'Accounts, credentials and dashboards that only a human can create. Every one of these blocks something; each has a proof of done so nobody assumes it happened.',
  ],
  [
    'BD',
    'Decisions waiting on you',
    'Product and business choices an agent must never invent. The mechanism gets built and tested either way — the value comes from here. BD-01 (escrow) unblocks the most.',
  ],
  [
    'W0',
    'Platform foundation',
    'Everything needed before a single feature can be built: repo, local stack, CI, per-PR environments, flags, secrets, encryption. This is the path to a first deployment.',
  ],
  [
    'W1',
    'Shared contracts and domain foundation',
    'The seam every other slice builds against: error shapes, generated API client, core schema, money handling, state machines.',
  ],
  [
    'W2',
    'Identity and access',
    'Signing up, staying signed in, and enforcing what each kind of user is allowed to do.',
  ],
  [
    'W3',
    'Providers and discovery',
    'Provider profiles and portfolios, and the search that puts them in front of a nearby client.',
  ],
  [
    'W4',
    'Jobs and presupuestos',
    'Posting a job, receiving quotes, comparing them, and awarding the work.',
  ],
  [
    'W5',
    'Payments and subscriptions',
    'Taking money, paying professionals, refunds, disputes, subscriptions and the ledger behind them. Highest risk slice.',
  ],
  [
    'W6',
    'Auctions',
    'Competitive bidding with a deadline, closing exactly once and awarding correctly.',
  ],
  [
    'W7',
    'Emergency call-outs',
    'Urgent requests broadcast to nearby available professionals, where exactly one wins the job.',
  ],
  [
    'W8',
    'Trust: licences, badges and reviews',
    'Verifying credentials, awarding badges that mean something, and keeping reviews honest.',
  ],
  [
    'W9',
    'Back office',
    'What an operator needs to run the marketplace day to day, with everything recorded.',
  ],
  [
    'W10',
    'Quality and launch',
...
const groupOf = (id: string) =>
  id.startsWith('OPS') ? 'OPS' : id.startsWith('BD') ? 'BD' : id.split('-')[0];
  ...
  const machinery =
    g === 'OPS' || g === 'BD'
      ? `Reference: \`TODO.md\` ${g === 'OPS' ? '§6 OPS table' : '§10.1'}\n` +
        (g === 'OPS'
          ? 'Human only — an agent must never attempt this. Identity rules: `docs/board/IDENTITY.md`.\n'
...
for (const [id, s] of Object.entries(stories)) {
  ...
  if (dry) {
    if (updated < 2) console.log(`--- ${id} — ${s.title}\n${body(id, s)}`);
    updated++;
    continue;
  }
...
console.log(
  `rewrote ${updated} issue(s)${missing.length ? `, missing: ${missing.join(', ')}` : ''}`,
);
if (dry) process.exit(0);

// --- epics
const refreshed: Row[] = JSON.parse(
  gh([
    'issue',
    'list',
    '--repo',
...
for (const [key, name, blurb] of EPICS) {
  ...
  if (existing) {
    gh(
      ['api', `repos/${repo}/issues/${existing.number}`, '-X', 'PATCH', '--input', '-'],
