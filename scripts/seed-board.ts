...
const MILESTONE_OF: Record<string, string> = {
  W0: 'M0 Walking skeleton', W1: 'M1 Identity', W2: 'M1 Identity', W3: 'M2 Discovery',
  W4: 'M4 Presupuestos', W5: 'M3 First euro', W6: 'M6 Auctions', W7: 'M7 Emergency',
  W8: 'M5 Trust', W9: 'M9 Beta-ready', W10: 'M9 Beta-ready',
};
...
for (const m of md.matchAll(/^\|\s*`(OPS-\d+)`\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|$/gm)) {
  ...
  issues.push({
    id,
    title: `${id} — ${trim(op)}`,
    body: `**Manual setup operation — human only.** Agents must never attempt this.\n\n` +
      `**Operation:** ${op}\n\n**Proof of done:** ${proof}\n\n**Unblocks:** ${unblocks}\n\n` +
...
for (const m of md.matchAll(/^\|\s*`(BD-\d+)`\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|\s*(.+?)\s*\|$/gm)) {
  ...
  issues.push({
    ...
    body: `**Business decision — owner: repo owner. Do not let an agent invent this.**\n\n` +
      `**Decision:** ${decision}\n\n**Blocks:** ${blocks}\n\n**Notes:** ${notes}\n\n` +
...
for (const m of md.matchAll(/^- `(W(\d+)-T\d+)`((?:\s*`\[[HMAB]\]`)+)\s*(.+)$/gm)) {
  ...
  const labels = [
    'type:task', `slice:${slice}`, `agent:${agent}`,
  ...
  const clean = text.replace(/\*\((human:.*?)\)\*/g, '').replace(/\*\*/g, '').trim();
  issues.push({
    id,
    title: `${id} — ${trim(clean)}`,
...
const LABELS: [string, string, string][] = [
  ...
  ...Object.values(SLICE_OF).map(([s]) => [`slice:${s}`, 'BFD4F2', `Slice ${s}`] as [string, string, string]),
  ...Object.values(SLICE_OF).map(([, a]) => [`agent:${a}`, 'D4C5F9', a] as [string, string, string]),
];
...
if (dryRun) {
  ...
  console.log(`labels: ${new Set(LABELS.map((l) => l[0])).size}, milestones: ${new Set(MILESTONES).size}`);
  console.log(`issues: ${issues.length}  (OPS ${by('OPS')}, BD ${by('BD')}, tasks ${issues.length - by('OPS') - by('BD')})`);
  console.log(`business-blocked: ${issues.filter((i) => i.labels.includes('decision:business')).length}`);
  console.log('\nfirst 8:');
...
for (const [name, color, desc] of new Map(LABELS.map((l) => [l[0], l])).values()) {
  try { sh('gh', ['label', 'create', name, '--repo', repo, '--color', color, '--description', desc, '--force']); }
  catch { console.warn(`label ${name}: skipped`); }
}
for (const title of new Set(MILESTONES)) {
  try { sh('gh', ['api', `repos/${repo}/milestones`, '-f', `title=${title}`]); }
  catch { /* already exists */ }
}

const existing = new Set(
  JSON.parse(sh('gh', ['issue', 'list', '--repo', repo, '--state', 'all', '--limit', '500', '--json', 'title']))
    .map((i: { title: string }) => i.title.split(' ')[0]),
);

let created = 0;
for (const i of issues) {
  if (existing.has(i.id)) { console.log(`= ${i.id} exists`); continue; }
  ...
  try { sh('gh', a); created++; console.log(`+ ${i.title}`); }
  catch (e) { console.error(`✖ ${i.id}: ${(e as Error).message.split('\n')[0]}`); }
}
