...
- `W12-T15` `[A]` Performance budget as a gate: Lighthouse CI on the preview URL, asserting **per-metric score floors (≥85) on a realistic 4G profile** — 10 Mbps / 40 ms RTT / 4× CPU, *not* Lighthouse's default mobile profile. **The `≤170 KB initial JS` budget is deleted by operator decision (2026-09-12), not relaxed**: the same build scores 68 under the default profile and 100 under a real-4G one without a byte changing, and `TBT ≈ 0` on both — the bundle costs transfer time on a link this product is not for, never main-thread time. A future byte budget must be re-argued from a measurement, never cited from this line. Lighthouse's SEO category stays deferred (ADR-011 Amendment 1). The R2 image pipeline (`srcset`/AVIF) and the font strategy split out — the latter is blocked on `W12-T19` choosing a face *(spec: `docs/specs/S10/W12-T15-performance-budget.md`, issue #215)*
