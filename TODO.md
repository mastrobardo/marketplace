...
- `W0-T23` `[A]` Stop parallel agents colliding on shared append-only files — namespaced i18n catalogues, one-record-per-file memory, README fragments *(issue #153)*. ⚠ **The i18n half needs redesigning before it can be built.** `W12-T21` made the catalogues JSON and removed the `TranslationKey` union and `satisfies Translations` that this ticket's §4 preserves — so its proposed barrel no longer describes anything that exists. **The problem is untouched and marginally worse**: seven agents still append to two files, and JSON has no comments to anchor a merge. Ask first whether a translation service moots it by taking the files out of the repo
...
- `W0-T27` `[A]` **Mirror the stack's third-party images into GHCR.** `minio/minio` and `minio/mc` were deleted from Docker Hub — the repositories, not the tags — and `stack:up` failed on every PR and on `main` until they were repointed at `quay.io` (#237). GHCR needs no external account, so this is `[A]`: the built-in `GITHUB_TOKEN` pushes to a package this repo owns. Both `linux/amd64` (CI) and `linux/arm64` (every laptop) must survive the copy. ⚠ **"Not urgent" was wrong, and #292 is the proof.** That judgement rested on #237 pinning by **digest**, so a re-pointed tag fails loudly — but the failure mode that keeps happening is the repository being *withdrawn*, which no pin survives: MinIO closed quay.io on 2026-09-26 and the stack broke a second time. A mirror is the only measure that would have made either outage a non-event, so this row is now the likely answer to `W0-T33` rather than a nice-to-have — and load-bearing at the same moment `W0-T26` is: an agent treating "CI green" as its success signal cannot diagnose a build that will not start because someone else deleted a tag *(issue #238)*
...
- `W0-T33` `[A]` `[B]` **TOP PRIORITY — a local object store this repo can actually pull.** *Operator, 2026-09-26: top priority once the stack is unblocked.* MinIO has now closed **every** public tap: `#237` moved these images off Docker Hub because the repositories were gone, and `quay.io/minio/{minio,mc}` — MinIO's own registry — now answers `unauthorized: access to the requested resource is not authorized` to an anonymous pull, while `ghcr.io/minio/*` answers 403. Verified by hand against all three token endpoints: quay **issues** an anonymous token and then refuses the manifest, which is what makes this read like a flaky CI job rather than a repository that stopped being public. Every `pnpm stack:up` failed — on CI, on `main`, and on a fresh clone. **The unblocking change pinned Bitnami's frozen builds** (`bitnamilegacy/minio`, `bitnamilegacy/minio-client`, July 2025, multi-arch, public) and that is a stopgap with no security patches, ever — which is the whole reason this row exists. **What it has to decide:** whether to authenticate against quay with a robot account (keeps real MinIO; costs every developer a credential and ends *clone and run*), swap to a different S3-compatible server (`chrislusf/seaweedfs`, `adobe/s3mock` and `localstack/localstack` are all still public — but a *mock* means `W6` develops against something that is not what runs in production), mirror into a registry this project owns — which is **`W0-T27` (#238), already filed and already deprioritised once on reasoning this outage falsified**, and which now has the wrinkle that you cannot mirror what you cannot pull, so it would be mirroring the frozen Bitnami build unless it is paired with one of the options above — or **drop object storage from the local stack until `W6` needs it** — nothing in the product touches it today and only `tests/local-stack.test.ts` asserts it is there. `[B]` because the first and third options need an account somebody owns. **Whatever it chooses, the lesson is already paid for three times: an upstream public image is not infrastructure** (`MEM-2026-09-17-8`, amended). Resolve this **with `W0-T27`, not beside it**
...
- `W4-T03` `[A]` ✅ **One quote for the whole job — the question `W4-T01` refused to prejudge.** Operator, 2026-09-20: *"1 quote for everything […] It would be also too chaotic for a user accepting X presupuestos: i look to remodel a bathroom, i want a quick way to get the job done."* So `Quote` references a `Job`, never a `JobCategory`, and **subcontracting becomes normal and invisible** — the plumber brings their electrician, and the review, flag and ban attach to whoever quoted (`MEM-2026-09-20-21`). **The platform does not gate who may quote**: *"It is up to the professional to decide to apply or not."* What it does instead is state the gap — `coverage` names every category the *job* asked for, whether the provider lists it, and whether it is licensed, which makes "quoting a gated trade you aren't verified for" visible rather than forbidden. **It carries no `verified` field**, and that is the point: nothing in this schema knows one (`W8` is unbuilt), so a nullable one would be `W4-T01`'s empty promise again. `POST /api/jobs/:id/quotes`, `GET /api/jobs/:id/quotes` (owner), `GET /api/me/quotes`, withdraw — **two readers, two permissions**, so "can a provider see a competitor's price?" is a permission question rather than a `WHERE` clause. One active quote per pro per job is a **partial** unique index (`WHERE status = 'PENDING'`): a plain one would make a withdrawn quote a permanent bar, and the predicate changes meaning the moment `W4-T04` adds states (`MEM-2026-09-20-22`). **Expiry is arithmetic, not a state** — no `EXPIRED` row, no worker, because a status that only becomes true when a sweep runs is wrong between the sweeps. `canQuoteOn` is a `Record<JobStatus, boolean>` and **caught its own dependency**: it would not compile on `main`, because `CANCELLED` is still on #277 *(spec: `docs/specs/S4/W4-T03-quote-submission.md`)*
...
- `W8-T05` `[A]` Reviews: only after a completed paid booking, both directions, edit window. **A review carries a satisfaction score, up to one competence score per trade the job named, and any positive tags — all optional** (ADR-015 §2): forcing three scores to get one manufactures opinions that do not exist, which is worse than missing data because it looks like data. **Attribution follows the *job's* categories, not the provider's claimed ones**, so a provider who quotes a trade they never listed still accrues its score — which is what makes it hard to game by editing a profile. **Also a revenue control, and labelled so it is not relaxed by accident** (ADR-013 §9): a professional's reputation only grows through on-platform work, which turns leaking from free money into a cost they pay themselves. *"Let clients review any job, we need more reviews"* is a reasonable-sounding growth proposal that would remove it silently
- `W8-T06` `[M]` `[B]` Rating aggregation + display rules *(human: `N`, the tag vocabulary, and the display policy)*. **Shape is ADR-015: two scores that are allowed to disagree.** *Satisfaction* with the person, shown as given; *competence* **per trade**, scored in its own row — because *"a score that catch all might be easier to read, but hide real bottlenecks"* (operator, 2026-09-20). A trade with no scores is **absent from every average**, never a zero and never 2.5. Where an aggregate is shown it is the **mean of the per-trade means**, not of all reviews: 5.0 over 50 plumbing jobs and 3.0 over 2 electrical ones is 4.0, not 4.92, and the second buries the bottleneck. Plus **positive-only tags** (*tidy*, *quiet*, *on time*) that aggregate into counts, because *"the best between 2 4 stars is the one that is stated as the more silent"* is a question no ranking can answer — the weighting lives in the client's circumstances. **The tag vocabulary is effectively permanent** (ADR-015 §4.1): old reviews cannot be retro-tagged, so a tag added later reads as a weakness on every established provider
...
- `W12-T21` `[A]` ✅ **Translations are data, not code.** `es.ts`/`en.ts` → `es.json`/`en.json`. Operator, 2026-09-20: *"translations should go on jsons. Which, in a future, might come from some online service. Having them in the code is a nogo."* **The compile-time key union went with them, and it did not have to** — `keyof typeof es` yields the same literal union from a JSON module under `resolveJsonModule` + `moduleResolution: Bundler`, and a two-way `Exact<>` type reproduces `satisfies Translations`; both were verified against `tsc` and then **rejected on purpose**, because if a translation service is going to own the catalogue its keys are not knowable at compile time, and a union derived from the checked-in snapshot is true only until the first fetch. *"It is fine to have extensive guards and typechecks, but this is not one of those occasions."* What guards a missing translation now already existed and survives the data arriving over HTTP, which the compiler could not: key-set parity and non-empty values in `tests/i18n.test.ts`, plus `missingKeyHandler` throwing at runtime. **Three fixture projects and the three tests that shelled out to `tsc` went with the mechanism they proved** — the web suite dropped from **16.6s to 5.3s**, each having carried a 120s timeout and a full `tsc`. All 220 keys per language were converted by importing the modules and serialising them, never retyped. **It leaves `W0-T23` without a design** and its problem intact: seven slice agents still append to two files, and JSON merges slightly worse than TypeScript (`MEM-2026-09-20-33`) *(spec: `docs/specs/S10/W12-T21-translations-as-json.md`)*
...
| `BD-10` | Badge qualifying rules · the **tag vocabulary** · minimum reviews before an average is shown | `W8-T04`, `W8-T06` | **Widened 2026-09-20 by ADR-015.** The threshold is now **per trade**, and there is more than one average. The tag list is the expensive half: it cannot be changed later without every established provider appearing to lack whatever was added, because old reviews cannot be retro-tagged (§4.1) |
