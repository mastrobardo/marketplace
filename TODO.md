...
> ### ▶ NEXT — `W0-T30` then `W3-T11`: get seed data into a deployed database, then let the back office write the taxonomy
>
> **Operator, 2026-09-19: both.** `W0-T30` first, because it is the blocker — *no* seeder currently
> reaches preview or staging, so `GET /api/categories` answers `{items: []}` there — and `W3-T11`
> makes the back office the taxonomy's writer.
>
