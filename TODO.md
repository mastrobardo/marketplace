...
> ### ▶ NEXT — `W12-T20`, then `W3-T05`/`W3-T07`
>
> `W2-T09` shipped the account pages, and building them surfaced a one-line defect that makes every
> page in the storefront look unfinished: **`apps/web` never imports `@marketplace/ui/styles.css`**,
> so the design system's component CSS is loaded by nothing and every React Aria control renders as
> a raw browser widget. That is `W12-T20`, and it is the cheapest visible improvement in the repo.
>
> After that, the endpoints that make the existing pages real — `W3-T05` (`GET /search`) and
> `W3-T07` (`GET /providers/:id`) — because the storefront is still a facade over MSW everywhere
> except auth.
>
> **A human blocker worth knowing about:** `OPS-14` (an email provider). Until it lands, signup works
> end to end **locally only** — in preview and staging the mail throws, and the account is stranded.
...
- `W12-T20` `[A]` **The storefront never loads the design system's stylesheet.** `apps/web/src/main.tsx` imports `@marketplace/ui/tokens.css` and `src/styles/app.css`; nothing imports `@marketplace/ui/styles.css` — the `dist/ui.css` export `W12-T02` §4 created for exactly this. So every React Aria component in the application renders as a raw browser control: the header's compact search, every `Button`, every `TextInput`. It has been this way since `W12`, and no suite noticed because the unit tests assert semantics and the visual baselines are *Storybook* screenshots, where the CSS is loaded. Found by `W2-T09` and **confirmed by experiment** — adding the one import turns `/es/signup` from raw inputs into the designed system. The fix is one line; the work is the review, because it changes the appearance of every existing page and every visual baseline. A gate belongs with it: an axe/visual pass over a *route* would have caught this, which is `W12-T16`'s route list plus a screenshot
