...
- `W0-T27` `[A]` 🟡 **Half-landed (2026-09-26, with `W0-T33`).** `.github/workflows/mirror-images.yml` + `docker/mirror.images` copy every stack image (`postgis`, `mailpit`, `garage`, `alpine`) into `ghcr.io/mastrobardo/marketplace/…` with the built-in `GITHUB_TOKEN`, server-side via `buildx imagetools`, failing the run if a copy loses `linux/amd64` or `linux/arm64`. **What is left is not code**: a GHCR package is created private and `GITHUB_TOKEN` cannot change that, so the four packages need one visibility flip each (*Settings → Packages → Change visibility*) before `docker-compose.yml` and `docker/garage/Dockerfile` can point at the copies — repointing first would pass CI, which has a token, and break every clone, which does not (`MEM-2026-09-26-5`). ⚠ This row was deprioritised once on the reasoning that digest pinning makes a re-pointed tag fail loudly; that was aimed at the wrong failure, and three withdrawals later a copy we hold is the only measure that would have prevented any of them
