...
- `W0-T27` ✅ **Done (2026-09-26, with `W0-T33`)** — every image the stack is made of is now pulled from `ghcr.io/mastrobardo/marketplace/…`: `docker-compose.yml` for `db` and `mail`, `docker/garage/Dockerfile` for both of its `FROM` lines. `.github/workflows/mirror-images.yml` makes the copies from `docker/mirror.images` with the built-in `GITHUB_TOKEN`, server-side via `buildx imagetools`, and refuses a copy that lost `linux/amd64` or `linux/arm64` or that differs from its upstream **digest** — which it does not: `imagetools create` copies the manifest, so each pin is the upstream digest and verifies the copy on every pull. ⚠ Two things this row predicted wrongly, both now measured: the packages needed **no** visibility flip (public on the first push, because Actions-published packages linked to a public repo inherit it — `MEM-2026-09-26-6`), and the first run went red not from a bad copy but from `grep -q` under `pipefail` killing `docker` with SIGPIPE (`MEM-2026-09-26-7`). This row was deprioritised once on the reasoning that digest pinning makes a re-pointed tag fail loudly; that was aimed at the wrong failure, and three withdrawals later a copy we hold is the only thing that would have prevented any of them
...
- `W0-T33` ✅ **Done (2026-09-26)** — the local object store is **Garage**, and MinIO is not coming back: `dl.min.io` answers **410 Gone** ("the open-source MinIO Server, MinIO Client (mc) and MinIO KES projects are archived and no longer maintained… does not provide security updates") and both GitHub repositories report `"archived": true`. That retired two of the four options this row was filed with before any code was written — **a quay robot account buys an archived server**, and there is no released binary left to build an image from. Operator chose Garage over SeaweedFS, a real R2 dev bucket, and wrangler's experimental local R2 (`wrangler@4.115.0`, July 2026); Durable Objects were raised and are the wrong primitive — stateful compute behind a Worker binding, no S3 endpoint. Also **no AWS, ever, in this project** (operator) — so provisioning is Garage's own CLI, not `amazon/aws-cli`. Spec + run record: `docs/specs/S0/W0-T33-object-store.md`. `W0-T27`'s mirror workflow landed with it and stays open until compose is repointed at the copies — see that row
